...
export default function MoviesByRating() {
	...
	const fetchMovies = async () => {
		try {
			setLoading(true);
			setError('');
			const data = await loadMoviesAPI({ sortBy: 'rating', order: 'desc' });
			setMovies(data);
		} catch (e) {
	...
	const { toggleWatched, deleteMovie } = useMovieListActions({ movies, setMovies, listType: 'rating' });
	...
	return (
		<div className="bg-white dark:bg-gray-900 dark:text-gray-100 rounded-xl p-4 shadow">
			<h2 className="font-bold mb-3">Filmes por Avaliação</h2>
